...
phh1 <- NULL; # List by chromosome : Section separate by position, 125 kb
phh2 <- NULL; # List by chromosome : Section with full info for chromosome
hhp1 <- NULL # List by chromosome : chromosome number + section pairs
for (i in 1:10) {
...
psss <- function(puu1, puu2) {
  ...
  phh3 <- list(); # indexes of
  phh4 <- NULL; # sum of pos_link for SNPs who is in 'i' kilobase
  ...
  phh5 <- cbind(puu1, phh4) # i - kilobase, sum of pos_link in this kilobase
  phh6 <- cbind(1:nrow(phh5), phh4) # num of kilobase, sum of pos_link
...
final_pos <- do.call(rbind, hhp1) # all chromosomes, for each of them - set of sections by 125 kilobase
...
for (i in 1:10) {
  ...
  ttt2[[i]] <- cbind(i, ttt1[[i]], pjjone[[i]][, 2]) # num of chr, num of kb, 2, 1, sum of pos_link for this kb
}
...
uuu <- function(rrr) {
  ...
  if (oo == 1) {
    ...
    if (se == 1) {
      fu <- c(rrr[1:fk_1, 2], rrr[(fk_1 + 1):nrow(rrr), 3])
    }
    else {
  ...
  if (oo == 2) {
    ...
    se <- sample(c(1, 2), 1)
    if (se == 1) {
      fu <- c(rrr[1:pp3[1], 2], rrr[(pp3[1] + 1):pp3[2], 3], rrr[(pp3[2] + 1):nrow(rrr), 2])
    }
    else {
  ...
  if (oo == 3) {
    ...
    se <- sample(c(1, 2), 1)
    if (se == 1) {
      fu <- c(rrr[1:pp3_1[1], 2], rrr[(pp3_1[1] + 1):pp3_1[2], 3], rrr[(pp3_1[2] + 1):pp3_1[3], 2], rrr[(pp3_1[3] + 1):nrow(rrr), 3])
    }else {
...
lilinone <- function(tttt) {
  ...
  le_2 <- NULL
  for (i in 1:10) {
    le_1[[i]] <- lele[which(lele[, 1] == i),] # section for one chromosome
    le_2[[i]] <- le_1[[i]][, 2:5] # delete 1 column with chr number
  }
  geng <- NULL;
  geng1 <- NULL;
  geng2 <- NULL
